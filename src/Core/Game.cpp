      ...
      m_InteractText(m_Font)
{
    ...
    // WORLD ALIGNMENT FIX: Shift walls by +0.5f to match physics center
    m_WallTransforms.clear();
    for (int x = 0; x < m_Map->GetWidth(); x++) {
        for (int z = 0; z < m_Map->GetHeight(); z++) {
            // Standard Walls (1) and Text Walls (9) use instancing
...
void Game::Update(float dt) {
    ...
    // --- AUDIO CONTROL: Stop on Death/Win ---
    if (m_State == GameState::GAME_OVER || m_State == GameState::WIN) {
        ...
        // Reset the static flag when we are playing so it triggers again next time
        static bool audioStopped = false;
        if(m_State == GameState::PLAYING) audioStopped = false;
    }

    if (m_State == GameState::PLAYING) {
        m_Player->HandleInput(m_Window, dt);
        ...
        // 2. Key Pickup Logic (Centered +0.5f correction)
        ...
        for (int x = 0; x < m_Map->GetWidth(); x++) {
            for (int z = 0; z < m_Map->GetHeight(); z++) {
                ...
                if (tile == 0 || tile == 3 || tile == 4 || tile == 9) {
                    ...
                    // LOW CEILING (2.4m)
                    model = glm::mat4(1.0f);
                    model = glm::translate(model, glm::vec3(x + 0.5f, 2.4f, z + 0.5f));
                ...
                // KEY (4): GLOWING BADGE
                if (tile == 4) {
                    m_Shader->SetBool("isUnlit", true); // DISABLE LIGHTING
...
void Game::RenderUI() {
    ...
    if (m_State == GameState::MENU) {
        ...
        m_UIText.setPosition({400.f, 200.f});
        m_Window.draw(m_UIText);

        // Click to Start
        // SFML 3.0: Use Button::Left
        ...
        if (m_Player->HasRedKey()) {
             ...
             keyIcon.setFillColor(sf::Color(255, 215, 0)); // Gold
    ...
    // 3. PAUSE MENU (Interactive)
        ...
        m_CenterText.setPosition({550.f, 200.f});
        m_Window.draw(m_CenterText);

        m_UIText.setString("[ESC] Resume\n[ENTER] Restart\n[Q] Quit Game");
        m_UIText.setPosition({500.f, 300.f});
